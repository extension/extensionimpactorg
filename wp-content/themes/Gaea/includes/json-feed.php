...
if ($events) {
		...
		$frequency_count = '';
			...
			$event_category_color = ($cat_data['catBG']!='')?$cat_data['catBG']:'';
			if($event_category_color == '#fff' || $event_category_color == '#ffffff'){
				$event_cat_color = '';
			} else {
				$event_cat_color = $event_category_color;
			}
			$event_color_default = $imic_options['event_default_color'];
			$recur_event_color = $imic_options['recurring_event_color'];
			if($frequency>0 && $event_cat_color != '') { $color = $event_cat_color; }
			elseif($frequency>0 && $recur_event_color != ''){ $color = $recur_event_color; }
			elseif($frequency>0) { $color = $event_color_default; }
        ...
        while ($fr_repeat <= $frequency_count) {
		    ...
		    $etime = date('Y-m-d H:i:s', strtotime($end));
			...
			'id' => $post->ID.'|'.$start,
		        ...
		        'start' => $stime,
		        'end' => $etime,
		        'url' => imic_query_arg($date_converted,$post->ID),
